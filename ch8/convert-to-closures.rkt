...
(provide convert-to-closures-R6)
...
(define (convert-to-closures e)
  (match e
    [`(has-type (inject ,e ,t) (,ta ... -> ,tr))
      (let-values ([(e clos) (convert-to-closures e)])
        (match e
          [`(has-type ,e* (Vector ,arg ...))
            (values
              `(has-type (inject ,e (Vectorof Any)) (Vectorof Any))
              clos)]
          [_ (values
               `(has-type (inject ,e ,t) (,@ta -> ,tr))
               clos)]))]
    [`(has-type (project ,e (,ta ... -> ,tr)) (Vector ,tv ...))
      (let-values ([(e clos) (convert-to-closures e)])
        (values
          `(has-type (project ,e (Vectorof Any)) (Vector ,tv ...))
          clos))]
    ...
    [`(has-type (app (has-type (project ,f (,ta ... -> ,tr)) ,tf) ,es ...) ,t)
      (let-values
        ...
        (let ([clo (gen-sym 'clo)]
          ...
          (values
            `(has-type
               (let ([,clo (has-type (project ,f (Vectorof Any)) ,tf*)])
                 (has-type
                   (app
                     (has-type
                       (vector-ref
...
(define convert-to-closures-R6 convert-to-closures)
